...
void TwoDimensionalPOMDP::writeToMatlabFile(string mapName){
	...
	outFile<<"% This file is auto created by the RLAI Temporal Difference Networks framework"<<endl;
	...
	for(int a=0;a<this->getActionCount();a++){
		outFile<<"res.transmat{1,"<<(a+1)<<"} = sparse("<<this->getStateCount()<<","<<this->getStateCount()<<");"<<endl;
		for(int s=0;s<this->getStateCount();s++){
			for(int s1=0;s1<this->getStateCount();s1++){
				float theProb=transitions[calcIndex(a,s,s1)];
				if(theProb>0.0f){
					outFile<<"res.transmat{1,"<<(a+1)<<"}("<<(s+1)<<","<<(s1+1)<<")="<<theProb<<";";
				}
			}
			outFile<<endl;
		}
	...
	//Also made this sparse to make the files manageable and readable
	outFile<<"res.obsmat = sparse("<<this->getStateCount()<<","<<this->getObservationCount()<<");"<<endl;
	...
	outFile<<"\%Mappings between nominal states and Row, Column, Direction"<<endl;
	outFile<<"res.map = [";
	for(int s=0;s<this->getStateCount();s++){
		outFile<<theMap.getRowForState(s)<<", "<<theMap.getColForState(s)<<", "<<theMap.getDirForState(s)<<";"<<endl;
	}
	outFile<<"];"<<endl;
...
void TwoDimensionalPOMDP::printCurrentBelief(){
	...
	if(probSum>1.0)
		cout<<"Added up probSums to: "<<probSum<<" means the belief state is corrupt"<<endl;
...
pomdpEnv(Map.countReachableStates(),Map.getOCount(),Map.getACount()),theMap(Map){

	cout<<"Just sent to pomdpEnv constructor: "<<Map.countReachableStates()<<" "<<Map.getACount()<<" "<<Map.getOCount()<<endl;
		...
		for(int i=0;i<2;i++)
			for(int j=0;j<2;j++)
				for(int k=0;k<2;k++){
					...
					float combinedProb=ObsProbs[RR][i]*ObsProbs[LL][j]*ObsProbs[FF][k];
...
//		if(SForward==SForwardForward){
	...
	//End of method
		...
		for(unsigned int i=0;i<theMap.getRows();i++){
			for (unsigned int j=0; j<theMap.getCols(); j++) {
