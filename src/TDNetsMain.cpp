...
float runExperiment(int totalSteps, int stepsPerBin, SolutionMethod *Alg,
		Environment *Env, bool printOutput=false);
//This runs an experiment to an error tolerance
float runExperimentTol(float tol, int stepsPerBin, SolutionMethod *Alg,
		Environment *Env, bool printOutput, int &numSteps, int &lastNumWrong);
...
int main(int argc, char * const argv[]) {
	...
	for (int i=0; i<numRuns; i++) {
		...
		for (int j=0; j<1; j++) {
			...
			float results=runExperimentTol(0.001f, stepsPerBin, Alg, Env,
					false, numSteps, lastNumWrong);
			allResultsFile<<"it "<<j<<" history Feature length: "<<hFL
					<<" weight Set length: "<<hWL<<" and depth: "<<d<< " is: "
					<<results<<" with "<<lastNumWrong<<" after "<<numSteps
					<<" iterations."<<endl;
			cout<<"Error hFL: "<<hFL<<" hWL: "<<hWL<<" d: "<<d<< " is: "
					<<results<<" wrong: "<<lastNumWrong<<" after it: "
					<<numSteps<<endl;
...
float runExperiment(int totalSteps, int stepsPerBin, SolutionMethod *Alg,
		Environment *Env, bool printOutput) {
...
float runExperimentTol(float tol, int stepsPerBin, SolutionMethod *Alg,
	...
	while ((prevBinError-curBinError)>tol) {
		...
		if (i%stepsPerBin==0) {
			if (printOutput) {
				cout<<"OE: "<<observedError/float(stepsPerBin)<<"\t numWrong: "<<numWrong<<"\tbin: "<<i
						/stepsPerBin<<endl;
			}
			...
			runningBinError = runningBinError + prevFinalBinError
					- finalBinError;
