...
void TDNet::Save(ofstream &o) const {
	if (DET)
		cout<<"Serializing TD Network Structure... all "<<Nodes.size()
				<<" nodes!"<<endl;
...
TDNet::TDNet(NodeFactory F, int depthValue, int actionCountValue,
		int obsCountValue) :
	depth(depthValue), actionCount(actionCountValue), obsCount(obsCountValue),
			theNodeFactory(F) {
...
void TDNet::HangNetworkOff(Node *parent, int Level) {
	for (int j=0; j<actionCount; j++) {
		...
		Nodes.push_back(theNodeFactory.createNode(j, obsToPredict, parent,
				Level));
